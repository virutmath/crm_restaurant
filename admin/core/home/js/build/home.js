...
var MenuList = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			'ul',
			{ className: 'list-unstyled list-menu' },
			rowLi
		);
	}
});
MenuList.CategoryItem = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			'li',
			{ className: 'list-item item-cat-parent' },
			React.createElement(
				'label',
				{ className: 'item-name', onClick: onClickFn },
			...
			React.createElement(
				'ul',
				{ className: 'list-cat-child list-unstyled', 'data-collapse-id': props.cat_id },
				rowMenu
			)
		);
	}
});
MenuList.CategoryItemChild = React.createClass({
	displayName: 'CategoryItemChild',
	...
	render: function render() {
		...
		return React.createElement(
			'li',
			{ className: 'list-item item-cat-child' },
			React.createElement(
				'label',
				{ className: 'item-name', onClick: onClickFn },
			...
			React.createElement(
				'ul',
				{ className: 'list-menu-child list-unstyled', 'data-collapse-id': props.cat_id },
				rowMenu
			)
		);
	}
...
MenuList.MenuItem = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			'li',
			{ className: 'list-item item-menu', 'data-id': props.men_id,
				'data-name': props.men_name },
			React.createElement(
				'label',
				{ className: 'item-name', onDoubleClick: onDbClick },
				'- ',
				props.men_name
			)
		);
...
var TableRow = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			...
			React.createElement(
				'tr',
				{ id: element_id, 'data-id': id, className: 'menu-desk-menu record-item', onClick: onClickFn },
				React.createElement(
					'td',
					{ className: 'center' },
					this.props.stt
				...
				React.createElement(
					'td',
					{ className: 'center' },
					React.createElement(FormattedNumber, { value: this.props.number })
				),
				React.createElement(
					'td',
					{ className: 'text-right' },
					React.createElement(FormattedNumber, { value: this.props.price, style: 'currency', currency: 'VND' })
				),
				React.createElement(
					'td',
					{ className: 'center' },
					React.createElement(FormattedNumber, { value: this.props.discount / 100, style: 'percent' })
				),
				React.createElement(
					'td',
					{ className: 'text-right' },
					React.createElement(FormattedNumber, { value: this.props.total, style: 'currency', currency: 'VND' })
				)
			)
		);
	}
});
HomeScript.react.TableMenu = React.createClass({
	...
	render: function render() {
		...
		return React.createElement(
			'table',
			{ className: 'table table-bordered table-hover table-listing', id: 'table-listing' },
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							...
							{ 'class': 'fl nowrap' },
							'T\u1ED5ng c\u1ED9ng ',
							HomeScript.currentDesk.menuList.length,
							' m\xF3n'
						)
					)
				)
...
HomeScript.view.adjustScreen = function () {
	...
	HomeScript.domElement.listingMenu.height(HomeScript.domElement.sectionContent.height() - $('#search-menu').height() - 180);
	...
	$.contextMenu({
		...
		items: {
			active: {
				...
				callback: function callback(key, opt) {
					var _this = $(this);
					if (_this.hasClass('active')) HomeScript.selectDesk(_this);else {
