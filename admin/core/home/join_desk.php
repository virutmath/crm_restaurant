...
//select bàn ăn cần ghép
$array_desk = array();
$sec_name = '';
    $db_query = new db_query('SELECT des_id,des_name,sec_name
                              FROM desks d
                              LEFT  JOIN  sections
...
//select list bàn ăn đã active có thể ghép được trừ id bàn ăn đang chọn
$list_desk = '';
$db_query_des = new db_query('SELECT
                                ...
                                WHERE
                                    ...
                                    AND des_id IN(' . rtrim($current_desk,',') . ')
                                    AND des_id <> '.$record_id.'
                                ORDER BY des_id ASC');
