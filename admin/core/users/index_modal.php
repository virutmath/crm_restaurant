...
if($cat_id && $isAjaxRequest){
    $db_extra_left_join = ' LEFT JOIN categories_multi ON use_group_id = cat_id '; 
    $db_extra_and       = 'AND cat_id = ' . $cat_id . ' ';
}else{
    $db_extra_left_join = '';
    $db_extra_and = '';
}
...
while($row = mysqli_fetch_assoc($db_listing->result)){
    $i++;
    if(!$row['use_code']){
        $staff_code = format_codenumber($row['use_id'],6,PREFIX_STAFF_CODE);
    }else {
        $staff_code = $row['use_code'];
    }
    $right_column .= $list->start_tr($i,$row[$id_field],'class="menu-normal record-item" ondblclick="addToParentWindow('.$row[$id_field].')" data-use_id="'.$row[$id_field].'" data-staff_name="'.$row['use_name'].'" data-staff_code="'.$staff_code .'"  onclick="active_record('.$row[$id_field].')" data-record_id="'.$row[$id_field].'"');
    /* code something */
...
<div class="col-xs-12">
    ...
    <span class="col-xs-8 text-right"><i>Bấm đúp chuột vào một khách hàng để chọn</i>&nbsp;&nbsp;&nbsp;&nbsp; <i class="fa fa-bell-o"></i> Chọn</span>
</div>';
